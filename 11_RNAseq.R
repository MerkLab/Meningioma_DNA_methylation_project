...
#get raw count data
setwd(here("data", "11_RNAseq"))
data_subset = read.csv(file = "raw_counts_MNG.csv")

#get meta data
meta_subset = read.csv(file="meta_subset.csv")
...
setwd(here("results"))
cairo_pdf(filename = "Screeplot.pdf",width = 16, height = 5)
...
#make PCA plots
setwd(here("results"))
cairo_pdf(filename = "PC1-2_cluster.pdf",width = 8, height = 8)
...
setwd(here("data", "11_RNAseq"))
data_Raleigh = read.table("Raleigh_RNAseq.txt", header=T)
...
setwd(here("data", "11_RNAseq"))
meta_Raleigh = read.table("Raleigh_meta.txt", header=T, row.names=1)
