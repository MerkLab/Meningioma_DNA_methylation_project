...
#read in files for discovery cohort
...
setwd(here("data", "09_discovery_CNV"))
...
setwd(here("results"))
tiff("Oncoprint_full_new.tiff", res=300,width=5120,height=2048)
...
by(GeIn_grade_T26$instability, GeIn_grade_T26$condition, shapiro.test)
#not normal, do not use ANOVA, but Kruskal-Wallis test
kruskal.test(instability ~ condition, data = GeIn_grade_T26)

#Kruskal Wallis test with multiple comparison Dunn test
dunnTest(instability ~ condition, data = GeIn_grade_T26, method = "bh")
...
#Shapiro wilks test for normality
by(GeIn_groups_T26$instability, GeIn_groups_T26$condition, shapiro.test)
#not normal, do not use ANOVA, but Kruskal-Wallis test
kruskal.test(instability ~ condition, data = GeIn_groups_T26)

#Kruskal Wallis test with multiple comparison Dunn test
dunnTest(instability ~ condition, data = GeIn_groups_T26, method = "bh")
...
GeIn_cluster_T26$condition = factor(GeIn_cluster_T26$condition, levels = c("METHlow", "METHhigh"))
str(GeIn_cluster_T26)
...
#Shapiro wilks test for normality
by(GeIn_cluster_T26$instability, GeIn_cluster_T26$condition, shapiro.test)
#Wilcoxon rank-sum test
wilcox.test(instability ~ condition, data = GeIn_cluster_T26)
...
#Shapiro wilks test for normality
by(GeIn_risk_T26$instability, GeIn_risk_T26$condition, shapiro.test)
#not normal, do not use ANOVA, but Kruskal-Wallis test
kruskal.test(instability ~ condition, data = GeIn_risk_T26)
#Kruskal Wallis test with multiple comparison Dunn test
dunnTest(instability ~ condition, data = GeIn_risk_T26, method = "bh")
...
dev.off()
#Shapiro wilks test for normality
by(CNVunfavor_grade_T26$CNV_unfavor, CNVunfavor_grade_T26$condition, shapiro.test)
#not normal, do not use ANOVA, but Kruskal-Wallis test
kruskal.test(CNV_unfavor ~ condition, data = CNVunfavor_grade_T26)

#Kruskal Wallis test with multiple comparison Dunn test
dunnTest(CNV_unfavor ~ condition, data = CNVunfavor_grade_T26, method = "bh")
...
dev.off()
#Shapiro wilks test for normality
by(CNVunfavor_group_T26$CNV_unfavor, CNVunfavor_group_T26$condition, shapiro.test)
#not normal, do not use ANOVA, but Kruskal-Wallis test
kruskal.test(CNV_unfavor ~ condition, data = CNVunfavor_group_T26)

#Kruskal Wallis test with multiple comparison Dunn test
dunnTest(CNV_unfavor ~ condition, data = CNVunfavor_group_T26, method = "bh")
...
dev.off()
#Shapiro wilks test for normality
by(CNVunfavor_risk_T26$CNV_unfavor, CNVunfavor_risk_T26$condition, shapiro.test)
#not normal, do not use ANOVA, but Kruskal-Wallis test
kruskal.test(CNV_unfavor ~ condition, data = CNVunfavor_risk_T26)

#Kruskal Wallis test with multiple comparison Dunn test
dunnTest(CNV_unfavor ~ condition, data = CNVunfavor_risk_T26, method = "bh")
...
CNVunfavor_cluster_T26$condition = factor(CNVunfavor_cluster_T26$condition, levels = c("METHlow", "METHhigh"))
...
dev.off()
#nomrmality test
by(CNVunfavor_cluster_T26$instability, CNVunfavor_cluster_T26$condition, shapiro.test)
#Wilcoxon rank-sum test
wilcox.test(instability ~ condition, data = CNVunfavor_cluster_T26)
...
#Shapiro wilks test for normality
by(x$instability, x$condition, shapiro.test)
#not normal, do not use ANOVA, but Kruskal-Wallis test
kruskal.test(instability ~ condition, data = x)
#Kruskal Wallis test with multiple comparison Dunn test
dunnTest(instability ~ condition, data = x, method = "bh")
...
#Shapiro wilks test for normality
by(x$CNV_favor, x$condition, shapiro.test)
#not normal, do not use ANOVA, but Kruskal-Wallis test
kruskal.test(CNV_favor ~ condition, data = x)
#Kruskal Wallis test with multiple comparison Dunn test
dunnTest(CNV_favor ~ condition, data = x, method = "bh")
...
#Shapiro wilks test for normality
by(x$instability, x$condition, shapiro.test)
#normal, but for better comparibility, use Kruskal-Wallis here as well
kruskal.test(instability ~ condition, data = x)
#Kruskal Wallis test with multiple comparison Dunn test
dunnTest(instability ~ condition, data = x, method = "bh")
...
########contextualize priorRT with genome instability and number of unfavorable CNVs in the entire cohort, and in clusters
###instability
#full discovery
setwd(here("data", "09_discovery_CNV"))
T26_prior_instability = read.csv("T26_priorRT_instability.csv", header = T)
T26_prior_instability$condition = factor(T26_prior_instability$condition, levels = c("prior_no", "prior_yes"))

setwd(here("results"))
cairo_pdf(filename = "T26_priorRT_instability.pdf", width = 3, height = 9)
ggplot(T26_prior_instability, 
       aes(condition, instability, fill=condition))+
  geom_jitter(height = 0, width = 0.1, color ="grey50")+
  geom_boxplot(outlier.shape = NA,alpha=0.8)+
...
dev.off()
#nomrmality test
by(T26_prior_instability$instability, T26_prior_instability$condition, shapiro.test)
#Wilcoxon rank-sum test
wilcox.test(instability ~ condition, data = T26_prior_instability)
...
setwd(here("data", "09_discovery_CNV"))
T26_cluster_prior_instability = read.csv("T26_cluster_priorRT_instability.csv", header = T)
T26_cluster_prior_instability$condition = factor(T26_cluster_prior_instability$condition, 
                                         levels = c("cluster1_prior_no", "cluster1_prior_yes","cluster2_prior_no","cluster2_prior_yes"))


df <- T26_cluster_prior_instability %>%
  mutate(
    xpos = case_when(
      condition == "cluster1_prior_no" ~ 1,
      condition == "cluster1_prior_yes" ~ 1.2,   # close to A
      condition == "cluster2_prior_no" ~ 1.7,    # gap between B and C
      condition == "cluster2_prior_yes" ~ 1.9    # close to C
    )
  )

df2 <- df %>%
  separate(condition,
           into = c("cluster", "prior1", "prior2"),
           sep = "_",
           remove = FALSE) %>%
  unite(prior, prior1:prior2, sep = "_")

setwd(here("results"))
cairo_pdf(filename = "T26_by_cluster_priorRT_instability.pdf", width = 6.5, height = 9)
ggplot(df2, 
       aes(x = cluster, y = instability, fill = prior)) +
  ...
  scale_fill_manual(values = c("prior_no" = "blue",
                               "prior_yes" = "red")) +
...
#Shapiro wilks test for normality
by(df$instability, df$condition, shapiro.test)
#not normal, do not use ANOVA, but Kruskal-Wallis test
kruskal.test(instability ~ condition, data = df)
#Kruskal Wallis test with multiple comparison Dunn test
dunnTest(instability ~ condition, data = df, method = "bh")

###test interaction of prior RT and genome instability
df2$cluster <- factor(df2$cluster)
df2$prior   <- factor(df2$prior, levels = c("prior_no", "prior_yes"))

art_model <- art(instability ~ cluster * prior, data = df2)
anova(art_model)
art_int <- artlm(art_model, "cluster:prior")

emmeans(art_int, pairwise ~ prior | cluster)


###unfavorable CNVs
#full discovery
setwd(here("data", "09_discovery_CNV"))
T26_prior_unfavor = read.csv("T26_priorRT_unfavor.csv", header = T)
T26_prior_unfavor$condition = factor(T26_prior_unfavor$condition, levels = c("prior_no", "prior_yes"))

setwd(here("results"))
cairo_pdf(filename = "T26_priorRT_unfavor.pdf", width = 3, height = 9)
ggplot(T26_prior_unfavor, 
       aes(condition, CNV_unfavor, fill=condition))+
  geom_jitter(height = 0, width = 0.1, color ="grey50")+
  geom_boxplot(outlier.shape = NA,alpha=0.8)+
  scale_fill_manual(values=c("blue","red")) +
  scale_y_continuous(breaks = seq(0,7.8,1), limits=c(0,7.8))+
  theme_classic()+
  theme(legend.position = "none")
dev.off()
#nomrmality test
by(T26_prior_unfavor$CNV_unfavor, T26_prior_unfavor$condition, shapiro.test)
#Wilcoxon rank-sum test
wilcox.test(CNV_unfavor ~ condition, data = T26_prior_unfavor)


#separated by cluster
#discovery by cluster
setwd(here("data", "09_discovery_CNV"))
T26_cluster_prior_unfavor = read.csv("T26_cluster_priorRT_unfavor.csv", header = T)
T26_cluster_prior_unfavor$condition = factor(T26_cluster_prior_unfavor$condition, 
                                                 levels = c("cluster1_prior_no", "cluster1_prior_yes","cluster2_prior_no","cluster2_prior_yes"))

df <- T26_cluster_prior_unfavor %>%
  mutate(
    xpos = case_when(
      condition == "cluster1_prior_no" ~ 1,
      condition == "cluster1_prior_yes" ~ 1.2,   # close to A
...
setwd(here("results"))
cairo_pdf(filename = "T26_by_cluster_priorRT_unfavor.pdf", width = 6.5, height = 9)
ggplot(df2, 
       aes(x = cluster, y = CNV_unfavor, fill = prior)) +
  ...
  scale_y_continuous(breaks = seq(0, 7.8, 1),
                     limits = c(0, 7.8)) +
...
#Shapiro wilks test for normality
by(df$CNV_unfavor, df$condition, shapiro.test)
#not normal, do not use ANOVA, but Kruskal-Wallis test
kruskal.test(CNV_unfavor ~ condition, data = df)

#Kruskal Wallis test with multiple comparison Dunn test
dunnTest(CNV_unfavor ~ condition, data = df, method = "bh")



###check instability and unfavorable CNVs only in recurrent meningiomas after prior RT
###instability
#recurrent cases
setwd(here("data", "09_discovery_CNV"))
T26_prior_instability = read.csv("T26_priorRT_instability_rec.csv", header = T)
T26_prior_instability$condition = factor(T26_prior_instability$condition, levels = c("prior_no", "prior_yes"))

setwd(here("results"))
cairo_pdf(filename = "T26_priorRT_instability_recurrent.pdf", width = 3, height = 9)
ggplot(T26_prior_instability, 
       aes(condition, instability, fill=condition))+
  geom_jitter(height = 0, width = 0.1, color ="grey50")+
  geom_boxplot(outlier.shape = NA,alpha=0.8)+
...
#recurrent cases
setwd(here("data", "09_discovery_CNV"))
T26_cluster_prior_instability = read.csv("T26_cluster_priorRT_instability_rec.csv", header = T)
T26_cluster_prior_instability$condition = factor(T26_cluster_prior_instability$condition, 
                                                 levels = c("cluster1_prior_no", "cluster1_prior_yes","cluster2_prior_no","cluster2_prior_yes"))
...
setwd(here("results"))
cairo_pdf(filename = "T26_by_cluster_priorRT_instability_recurrent.pdf", width = 6.5, height = 9)
ggplot(df2, 
       aes(x = cluster, y = instability, fill = prior)) +
...
#test interaction of cluster and prior RT
df2$cluster = factor(df2$cluster, levels = c("cluster1", "cluster2"))
df$priorRT = factor(df2$priorRT, levels = c("prior_no", "prior_yes"))

art_rec <- art(instability ~ cluster * priorRT, data = df2)
anova(art_rec)
...
#recurrent cases
setwd(here("data", "09_discovery_CNV"))
T26_prior_unfavor = read.csv("T26_priorRT_unfavor_rec.csv", header = T)
T26_prior_unfavor$condition = factor(T26_prior_unfavor$condition, levels = c("prior_no", "prior_yes"))

setwd(here("results"))
cairo_pdf(filename = "T26_priorRT_unfavor_recurrent.pdf", width = 3, height = 9)
ggplot(T26_prior_unfavor, 
       aes(condition, CNV_unfavor, fill=condition))+
  geom_jitter(height = 0, width = 0.1, color ="grey50")+
  geom_boxplot(outlier.shape = NA,alpha=0.8)+
...
#recurrent cases
setwd(here("data", "09_discovery_CNV"))
T26_cluster_prior_unfavor = read.csv("T26_cluster_priorRT_unfavor_rec.csv", header = T)
T26_cluster_prior_unfavor$condition = factor(T26_cluster_prior_unfavor$condition, 
                                             levels = c("cluster1_prior_no", "cluster1_prior_yes","cluster2_prior_no","cluster2_prior_yes"))
...
setwd(here("results"))
cairo_pdf(filename = "T26_by_cluster_priorRT_unfavor_recurrent.pdf", width = 6.5, height = 9)
ggplot(df2, 
       aes(x = cluster, y = CNV_unfavor, fill = prior)) +
